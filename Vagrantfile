...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "libvirt" do |libvirt|
	...
	config.vm.provision "shell", inline: <<-SHELL
		rpm --import https://dl.packager.io/srv/opf/openproject-ce/key
		yum install -y wget
		wget -qO /etc/yum.repos.d/openproject-ce.repo \
		https://dl.packager.io/srv/opf/openproject-ce/stable/7/installer/el/7.repo
		yum update -y
		yum install -y postgresql-server postgresql-contrib openproject git
		postgresql-setup initdb
	  cp pg_conf/pg_hba.conf /var/lib/pgsql/data/
	...
	config.vm.synced_folder "postgres_config", "/home/vagrant/pg_conf/"
